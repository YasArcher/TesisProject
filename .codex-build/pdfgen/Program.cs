...
static List<Block> Parse(string[] lines)
{
    ...
    for (var i = 0; i < lines.Length; i++)
    {
        ...
        if (line.TrimStart().StartsWith("```"))
        {
            ...
            else { result.Add(new CodeBlock(string.Join("\n", code))); inCode = false; }
            continue;
        }
        ...
        if (line.TrimStart().StartsWith("|"))
        {
            ...
            while (i < lines.Length && lines[i].TrimStart().StartsWith("|"))
            {
                ...
                var cells = current.Trim('|').Split('|').Select(x => x.Trim()).ToArray();
                var separator = cells.All(x => Regex.IsMatch(x, "^:?-{3,}:?$"));
                if (!separator) rows.Add(cells);
                i++;
            }
            i--;
            if (rows.Count > 0) result.Add(new TableBlock(rows));
            continue;
        }
...
static void RenderBlock(ColumnDescriptor col, Block block)
{
    ...
    {
        case HeadingBlock h:
            col.Item().PaddingTop(h.Level == 1 ? 0 : 8).Text(Clean(h.Text))
                .FontSize(h.Level == 1 ? 18 : h.Level == 2 ? 13 : 11)
                .Bold()
                .FontColor(h.Level == 1 ? "003638" : h.Level == 2 ? "055052" : "285A48");
            break;
        case ParagraphBlock p:
            col.Item().Text(Clean(p.Text)).FontSize(9.2f).LineHeight(1.18f);
            break;
        case CodeBlock c:
            col.Item().Background("F3F6F5").BorderLeft(3).BorderColor("408A71").Padding(6)
                .Text(c.Text).FontFamily("Consolas").FontSize(8.4f).FontColor("660B05");
            break;
        case TableBlock t:
            RenderTable(col, t.Rows);
            break;
    }
}
...
static void RenderTable(ColumnDescriptor col, List<string[]> rows)
{
    ...
    col.Item().Table(table =>
    {
        ...
        {
            for (var i = 0; i < columns; i++) def.RelativeColumn();
        });
        for (var r = 0; r < rows.Count; r++)
        {
            for (var c = 0; c < columns; c++)
            {
                ...
                if (r == 0)
                    cell.Background("003638").Text(value).FontColor(Colors.White).Bold().FontSize(8.2f);
                else
                    cell.Background(r % 2 == 0 ? "F1F7F4" : "FBFDFC").Text(value).FontSize(7.8f).LineHeight(1.1f);
            }
        }
    });
}
...
record HeadingBlock(int Level, string Text) : Block;
record ParagraphBlock(string Text) : Block;
record CodeBlock(string Text) : Block;
record TableBlock(List<string[]> Rows) : Block;
