...
namespace tesisproject.frontend.Services.Implementations
{
    public sealed class ApiClient : IApiClient
    {
        ...
        public async Task<HttpResponseWrapper<TResponse?>> PostAsync<TRequest, TResponse>(string url, TRequest body, CancellationToken ct = default)
        {
            try
            {
                var json = new StringContent(JsonSerializer.Serialize(body, _jsonOptions), Encoding.UTF8, "application/json");
        ...
        // ==================== CORE PARSER ====================
        private static async Task<HttpResponseWrapper<T?>> ParseResponseAsync<T>(HttpResponseMessage resp, CancellationToken ct)
        {
