...
public sealed class UnifiedArticleUserContext : IUnifiedArticleUserContext
{
    ...
    public UnifiedArticleUserContext(
    ...
    public string? OwnerReference => Principal.FindFirstValue(ClaimTypes.NameIdentifier) ?? Principal.Identity?.Name;

    public bool CanManageAll => IsAuthenticated && (IsInRole(AppRoles.Admin) || IsInRole(AppRoles.SuperAdmin)
        || IsInRole(ArticleRoles.Analyst) || HasPermission(ArticlePermissions.ManageConfiguration));
    ...
    public async Task<int?> GetAppUserIdAsync(CancellationToken ct = default)
    {
        ...
        var appUser = await _uow.AppUsers.GetByLocalIdAsync(identityUserId.Value, ct);
