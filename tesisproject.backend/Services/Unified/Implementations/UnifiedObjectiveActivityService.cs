...
{
    public class UnifiedObjectiveActivityService : IUnifiedObjectiveActivityService
    {
        ...
        public UnifiedObjectiveActivityService(IUnifiedUnitOfWork uow)
        {
            _uow = uow;
        }
            ...
            CancellationToken ct = default)
        {
            if (objectiveId <= 0)
                return ServiceResult<IReadOnlyList<ObjectiveActivityListItemDTO>>
                    .Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_ObjectiveIdRequiredMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);
            ...
            CancellationToken ct = default)
        {
            if (activityId <= 0)
                return ServiceResult<ObjectiveActivityDetailDTO>
                    .Fail(ErrorMessages.UnifiedLegacy.IndexingSourceService_InvalidIdMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);

            var entity = await _uow.ObjectiveActivities.GetByIdWithRefsAsync(activityId, ct);
            if (entity is null)
                return ServiceResult<ObjectiveActivityDetailDTO>
                    .Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_ObjectiveActivityNotFoundMessage, ErrorType.NotFound, ErrorCodes.Common.NotFound);
            ...
            CancellationToken ct = default)
        {
            if (request is null)
                return ServiceResult<ObjectiveActivityDetailDTO>
                    .Fail(ErrorMessages.UnifiedLegacy.FacultyScopeService_RequestRequiredMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);

            if (!request.ObjectiveId.HasValue || request.ObjectiveId.Value <= 0)
                return ServiceResult<ObjectiveActivityDetailDTO>
                    .Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_ObjectiveIdRequiredMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);

            // Validar que el ProjectObjective exista
            var objective = await _uow.ProjectObjectives.GetByIdAsync(
                new object[] { request.ObjectiveId.Value }, ct);

            if (objective is null)
                return ServiceResult<ObjectiveActivityDetailDTO>
                    .Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_ProjectObjectiveNotFoundMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);

            var entity = new ObjectiveActivity
            {
                ObjectiveId = request.ObjectiveId.Value,
            ...
            CancellationToken ct = default)
        {
            ...
            if (activityResult.Length > MaxTextLength)
                return ServiceResult<ObjectiveActivityDetailDTO>
                    .Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_ActivityResultMaxLengthMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);

            var actionText = (request.ActionText ?? string.Empty).Trim();
            if (actionText.Length > MaxTextLength)
                return ServiceResult<ObjectiveActivityDetailDTO>
                    .Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_ActionTextMaxLengthMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);
            ...
            CancellationToken ct = default)
        {
            ...
            if (entity is null)
                return ServiceResult<bool>.Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_ObjectiveActivityNotFoundMessage, ErrorType.NotFound, ErrorCodes.Common.NotFound);
        ...
        {
            if (visitId <= 0)
                return ServiceResult<bool>.Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_InvalidVisitIdMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);

            if (activityId <= 0)
                return ServiceResult<bool>.Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_InvalidActivityIdMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);

            if (progressPercentage < 0 || progressPercentage > 100)
                return ServiceResult<bool>.Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_ProgressRangeMessage, ErrorType.Validation, ErrorCodes.Common.InvalidRequest);

            // 1) Validar Visit
            var visit = await _uow.Visits.GetByIdAsync(new object[] { visitId }, ct);
            if (visit is null)
                return ServiceResult<bool>.Fail(ErrorMessages.UnifiedLegacy.ObjectiveActivityService_VisitNotFoundMessage, ErrorType.NotFound, ErrorCodes.Common.NotFound);
            ...
            if (progressPercentage < totalBeforeThisVisit)
            {
                return ServiceResult<bool>.Fail(
                    string.Format(ErrorMessages.UnifiedLegacy.ObjectiveActivityProgressBelowPreviousVisit, totalBeforeThisVisit),
