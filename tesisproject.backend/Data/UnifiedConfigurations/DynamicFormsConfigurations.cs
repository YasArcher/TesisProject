...
public sealed class ProductAuthorDynamicFieldValueConfiguration : IEntityTypeConfiguration<ProductAuthorDynamicFieldValue>
{
    public void Configure(EntityTypeBuilder<ProductAuthorDynamicFieldValue> entity)
    {
        entity.ToTable("ProductAuthorDynamicFieldValues");
        entity.HasKey(x => x.ProductAuthorDynamicFieldValueId);
        entity.Property(x => x.ValueDecimal).HasPrecision(18, 4);

        entity.HasOne(x => x.ProductAuthor)
            .WithMany(x => x.DynamicFieldValues)
            .HasForeignKey(x => x.ProductAuthorId)
            .OnDelete(DeleteBehavior.Cascade);
